...
public class SslExceptionsWrapper {
    ...
    private static long getIdForAuthority(ContentResolver contentResolver, String authority) {
        String whereClause = SslExceptionsProvider.Columns.AUTHORITY + " = \"" + authority + "\"";
        Cursor cursor = contentResolver.query(SslExceptionsProvider.SSL_EXCEPTIONS_URI, SSL_EXCEPTIONS_PROJECTION, whereClause, null, null);
        CursorManager.SingleItemCursor<Long> longSingleItemCursor = new CursorManager.SingleItemCursor<Long>(new Function<Cursor, Long>() {
            @Override
            public Long apply(Cursor cursor) {
                return cursor.getLong(cursor.getColumnIndex(SslExceptionsProvider.Columns._ID));
            }
        });
        Long result = longSingleItemCursor.query(cursor);
        if (result != null) {
            return result;
        }
        return -1L;
